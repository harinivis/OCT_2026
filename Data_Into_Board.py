...
combined_bytes = np.hstack((data_bytes, data_complex_bytes))


print('length')
#print (len(data_bytes+data_complex_bytes))

print((data_bytes.shape))
print((data_complex_bytes.shape))
print((combined_bytes.shape))

print(data_bytes)
print(data_complex_bytes)
...
for i in range(num_scans):
    scan_combined = combined_bytes[i, :]  # go through each A scan up to 768 and go though all data points (512) in each a scan 
    ...
    number_of_samples = len(scan_combined) # does NumberofSamples = int16(length(scan)); 
    Data = scan_combined
    close_marker = 32767
...
#   network_byteorder_int = struct.pack(">H", 256) --> this packs the number 256
    combined_data_packet = struct.pack('>' + 'i' * number_of_samples, *scan_combined) # big endian, do one a scan at a time multiplied by 512, and, *scan does scan[0], scan[1], scan[2], so it packs all 512 scans 
    ...
    full_packet = basic_packet + combined_data_packet + close_packet 
