...
INTER_PACKET_DELAY_SEC = 0.0  # not entirely sure if packet backlog will 
...
ROW_SAMPLES = BATCH_SAMPLES * PACKETS_PER_ROW_TX #i.e 512
...
def build_header(row_id: int, batch_id: int, msg_type: int) -> bytes:
    ...
    print(f"Sending header=0x{header_word:08X}")
...
def parse_packet(pkt: bytes):
    ...
    if magic_hi != 0xFF or magic_lo != 0x03:
        raise ValueError(f"bad header magic: {magic_hi:02X} {magic_lo:02X}")
    #print(f"Header received: hi=0x{magic_hi:02X}, lo=0x{magic_lo:02X}")
    samples = []
    payload = pkt[4:]
...
def send_row_and_receive_reply(sock, row_id: int, row_samples, source = "raw"):
    ...
    for batch_id in range(PACKETS_PER_ROW_TX): #batch id is are we on first, second, third, etc batch
...
def do_cal():
    ...
    for row_samples, row_id in zip (calibration_rows, calib_row_ids):
        ...
        replies = send_row_and_receive_reply(sock, row_id, row_samples, source = "calib")
        print(f"Sent calibration row {row_id}, received {len(replies)}/{PACKETS_PER_ROW_RX} packets")
...
def main():
    ...
    #  #FOR THIS FILE, SEND EACH ROW IN THE BATCHES 
        for row_id, row_samples in enumerate(rows):
            ...
            replies = send_row_and_receive_reply(sock, row_id, row_samples, source = "raw")
            print()
            print(f"Received {len(replies)}/{PACKETS_PER_ROW_RX} reply packets")
            print()
